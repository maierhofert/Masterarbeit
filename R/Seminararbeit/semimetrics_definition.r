...
#############################################
# semimets: Semimetrik
# x: Daten (funktionale Kovariablen)
# tt: Zeit
# deg: Aktueller Grad der Ableitung
# l.D.small: Zu betrachtender Bereich der short_eucl Metrik 
#            als Folge der eingehenden Datenpunkte
# l.tt2: Zweiter Zeitvektor für neue Variable (unused) = tt
# l.ii: ist D_1 in relative areas (hängt mit L.D.small zusammen)
#       Teilt Fläche unter Bereich von L.D.small[[ii]] durch Fläche 
#       unter dem Bereich der in jj (= D_2 in relative areas) definiert wird 
# l.tvec Sprunghöhe t_n und t_o in d_jump

# l. heisst Liste
metric <- function(semimets, x, tt, deg, l.D.small, l.tt2, l.ii, l.tvec, ...) {
    metrics <- list()
    metrics <- do.call(semimets, args = list(x, tt, deg, l.D.small, l.tt2, l.ii, 
        l.tvec, ...))
    return(metrics)
}
...
# x Matrix der Kurve
# tt Zeit
# deg: ein integer der den aktuellen Ableitungsgrad festlegt

## Rückgabe
# Liste mit 
# distances: Liste von Vektoren der errechneten Distanzen der Semimetrik
#           Kann auch Liste von Liste von Distanzvektoren sein
# op.dist: Vektor von Abständen (aktuell ausschließlich equidistant implementiert)
metric.eucl <- function(x = x, tt = tt, deg = deg, ...) {
    metric.eucl <- list()
    ...
    # mit diff(tt) ersetzen
    if (is.null(nrow(x))) {
        metric.eucl$distances <- 0
    } else {
      # Berechne alle Ableitungen bis zum grad deg Punktweise
        if (deg >= 1) {
            for (a in 1:deg) {
                if (a == 1) {
      ...
      # op.dist erst hier definieren, da bei höheren Ableitungen Werte entfallen
...
#         # mit diff(tt) ersetzen
      ...
      # jetzt ist x die abgeleitete Kurve
        for (a in 1:(nrow(x))) { # iteriere durch die Kurven (= Zeilen)
            metric.eucl$distances[[a]] <- vector()
            # Für letzte Beobachtung
            if (a == nrow(x)) {
                inb <- t(apply(x[1:(a - 1), ], MARGIN = 1, FUN = function(x, y) {
                  x - y # Berechnung der eigentlichen Metrik
                }, y = x[a, ]))
                metric.eucl$distances[[a]] <- sqrt(metric.eucl$op.dist * rowSums(inb^2))
            } else { 
              # Für alle anderen Beobachtungen (nicht die letzte)
                inb <- t(apply(rbind(x[1:(a - 1), ], x[(a + 1):nrow(x), ]), MARGIN = 1, 
                  FUN = function(x, y) {
                    x - y # Berechnung der eigentlichen Metrik
                  }, y = x[a, ]))
                # Hier muss metric.eucl$op.dist elementweise mit jeder Zeile (Kurve) multipliziert werden
                # Ergibt Vektor mit Distanzen der Beobachtungen zu Kurve a
                metric.eucl$distances[[a]] <- sqrt(metric.eucl$op.dist * rowSums(inb^2))
            }
            # Für die erste Kurve
            if (a == 1) { 
                metric.eucl$distances[[a]] <- vector()
                inb <- t(apply(x[(a + 1):nrow(x), ], MARGIN = 1, FUN = function(x, 
                  y) {
                  x - y
                }, y = x[a, ]))
                metric.eucl$distances[[a]] <- sqrt(metric.eucl$op.dist * rowSums(inb^2))
...
############################################# 
metric.shorteucl <- function(x = x, tt = tt, deg = deg, l.D.small = l.D.small, ...) {
    ...
    if (is.null(nrow(x))) {
        ...
        for (no.Dsmall in 1:length(l.D.small)) {
            ...
            for (a in 1:(nrow(x.inb))) {
                metric.shorteucl$distances[[no.Dsmall]][[a]] <- vector()
                if (a == (nrow(x.inb))) {
                  inb <- t(apply(x.inb[1:(a - 1), ], MARGIN = 1, FUN = function(x, 
                    y) {
                    x - y
                  }, y = x.inb[a, ]))
                  metric.shorteucl$distances[[no.Dsmall]][[a]] <- sqrt(metric.shorteucl$op.dist * 
                    rowSums(inb^2))
                } else {
                  inb <- t(apply(rbind(x.inb[1:(a - 1), ], x.inb[(a + 1):nrow(x.inb), 
                ...
                if (a == 1) {
                  metric.shorteucl$distances[[no.Dsmall]][[a]] <- vector()
                  inb <- t(apply(x.inb[(a + 1):nrow(x.inb), ], MARGIN = 1, FUN = function(x, 
                    y) {
                    x - y
                  }, y = x.inb[a, ]))
                  metric.shorteucl$distances[[no.Dsmall]][[a]] <- sqrt(metric.shorteucl$op.dist * 
...
############################################# 
metric.mean <- function(x = x, tt = tt, deg = deg, ...) {
    metric.mean <- list()
    ...
    if (is.null(nrow(x))) {
        ...
        for (a in 1:(nrow(x))) {
            ...
            if (a == (nrow(x))) {
                inb <- metric.mean$op.dist * rowSums(x[1:(a - 1), ]) - metric.mean$op.dist * 
                  sum(x[a, ])
            } else {
                inb <- metric.mean$op.dist * rowSums(rbind(x[1:(a - 1), ], x[(a + 
                  1):nrow(x), ])) - metric.mean$op.dist * sum(x[a, ])
            }
            if (a == 1) {
                rm(inb)
                inb <- metric.mean$op.dist * rowSums(x[(a + 1):nrow(x), ]) - metric.mean$op.dist * 
                  sum(x[a, ])
            }
...
############################################# 
metric.relareas <- function(x = x, tt = tt, deg = deg, l.D.small = l.D.small, l.tt2 = l.tt2, 
    l.ii = l.ii, jj = jj, ...) {
            ...
            if (length(l.D.small) < max(l.ii[[no.ii]], jj)) 
                ...
                for (a in 1:(nrow(x))) {
                  ...
                  if (a == (nrow(x))) {
                    ...
                    inb5 <- sum(inb[a, ])
                    inb6 <- sum(inb2[a, ])
                    metric.relareas$distances[[no.ii]][[a]] <- sqrt((abs(inb3/inb4) - 
                      abs(inb5/inb6))^2)
                  } else {
...
############################################# 
metric.jump <- function(x = x, tt = tt, deg = deg, l.D.small = l.D.small, l.tt2 = l.tt2, 
    l.ii = l.ii, l.tvec = l.tvec, ...) {
    metric.jump <- list()
...
############################################# 
metric.max <- function(x = x, tt = tt, deg = deg, ...) {
    metric.max <- list()
...
############################################# 
metric.min <- function(x = x, tt = tt, deg = deg, ...) {
    metric.min <- list()
...
############################################# 
metric.points <- function(x = x, tt = tt, deg = deg, point.list = point.list, no.sim = no.sim, 
    ...) {
    metric.points <- list()
...
############################################# 
metric.scan <- function(x = x, tt = tt, deg = deg, phi.func = phi.func, no.sim = no.sim, 
    ...) {
    metric.scan <- list()
    ...
    if (is.null(nrow(x))) {
        ...
        for (b in 1:length(l.t.l)) {
            ...
            ## lines(tt,phi.inb[[b]],col=1,lwd=.1)
            for (a in 1:(nrow(x))) {
...
metric.2.eucl <- function(x = x, tt = tt, deg = deg, ...) {
    metric.2.eucl <- list()
...
############################################# 
metric.2.shorteucl <- function(x = x, tt = tt, deg = deg, l.D.small = l.D.small, 
    ...) {
        ...
        for (no.Dsmall in 1:length(l.D.small)) {
            ...
            for (a in 1:(nrow(x.inb))) {
                ...
                if (a == (nrow(x.inb))) {
                  ...
                  metric.2.shorteucl$distances[[no.Dsmall]][[a]] <- sqrt(metric.2.shorteucl$op.dist * 
                    rowSums(inb^2))
                } else {
                  inb <- t(apply(rbind(x.inb[1:(a - 1), ], x.inb[(a + 1):nrow(x.inb), 
                    ]), MARGIN = 1, FUN = function(x, y) {
...
############################################# 
metric.2.mean <- function(x = x, tt = tt, deg = deg, ...) {
    metric.2.mean <- list()
    ...
    if (is.null(nrow(x))) {
        ...
        for (a in 1:(nrow(x))) {
            ...
            if (a == (nrow(x))) {
                inb <- metric.2.mean$op.dist * rowSums(x[1:(a - 1), ]) - metric.2.mean$op.dist * 
                  sum(x[a, ])
            } else {
                inb <- metric.2.mean$op.dist * rowSums(rbind(x[1:(a - 1), ], x[(a + 
                  1):nrow(x), ])) - metric.2.mean$op.dist * sum(x[a, ])
            }
            if (a == 1) {
                rm(inb)
                inb <- metric.2.mean$op.dist * rowSums(x[(a + 1):nrow(x), ]) - metric.2.mean$op.dist * 
                  sum(x[a, ])
            }
...
############################################# 
metric.2.relareas <- function(x = x, tt = tt, deg = deg, l.D.small = l.D.small, l.tt2 = l.tt2, 
    l.ii = l.ii, jj = jj, ...) {
            ...
            if (length(l.D.small) < max(l.ii[[no.ii]], jj)) 
                ...
                for (a in 1:(nrow(x))) {
                  ...
                  if (a == (nrow(x))) {
                    ...
                    inb5 <- sum(inb[a, ])
                    inb6 <- sum(inb2[a, ])
                    metric.2.relareas$distances[[no.ii]][[a]] <- sqrt((abs(inb3/inb4) - 
                      abs(inb5/inb6))^2)
                  } else {
                    inb3 <- rowSums(rbind(inb[1:(a - 1), ], inb[(a + 1):nrow(inb), 
                      ]))
...
############################################# 
metric.2.max <- function(x = x, tt = tt, deg = deg, ...) {
    metric.2.max <- list()
...
############################################# 
metric.2.min <- function(x = x, tt = tt, deg = deg, ...) {
    metric.2.min <- list()
...
############################################# 
metric.2.points <- function(x = x, tt = tt, deg = deg, point.list = point.list, no.sim = no.sim, 
    ...) {
    ...
    if (is.null(nrow(x))) {
        ...
        for (a in 1:(nrow(x))) {
            ...
            if (a == nrow(x)) {
                # matplot(tt,t(x),type='l',ylab='x(t)')
                # lines(tt[pl.short],rep(max(x),length=length(pl.short)),lwd=2,col=deg+1,type='h')
                inb <- t(apply(x[1:(a - 1), ], MARGIN = 1, FUN = function(x, y) {
                  x - y
                }, y = x[a, ]))
                for (p in 1:length(pl.short)) if (p == 1) 
...
############################################## 
metric.2.scan <- function(x = x, tt = tt, deg = deg, phi.func = phi.func, no.sim = no.sim, 
    ...) {
    metric.2.scan <- list()
    ...
    if (is.null(nrow(x))) {
        ...
        for (b in 1:length(l.t.l)) {
            ...
            if (maxx != 0) 
                phi.inb[[b]] <- phi.inb[[b]] * (maxx/max(phi.inb[[b]]))
            # lines(tt,phi.inb[[b]],col=deg+1,lwd=.1)
            for (a in 1:(nrow(x))) {
                ...
                if (a == nrow(x)) {
                  ...
                  inb2 <- inb2^2
                  metric.2.scan$distances[[b]][[a]] <- sqrt(metric.2.scan$op.dist * 
                    rowSums(inb2))
                } else {
                  inb <- t(apply(rbind(x[1:(a - 1), ], x[(a + 1):nrow(x), ]), MARGIN = 1, 
                    FUN = function(x, y) {
