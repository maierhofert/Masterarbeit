...
#####################################################################
#####################################################################
my.funopadi.knn.lcv <- function(Classes, CURVES, PRED, ..., kind.of.kernel = "quadratic", semimetric = "deriv")
{
  ################################################################
  ...
  if(twodatasets) {
    ...
    return(list(Estimated.classnumber = Classes.estimated, 
                ...
                = Bandwidth.opt, Misclas = Misclas.estimated, Prob.predicted = PROB.PREDICTED))
  }else {
    return(list(Estimated.classnumber = Classes.estimated, 
                Bandwidths = Bandwidth.opt, Misclas = Misclas.estimated, Prob.predicted = PROB.PREDICTED))
  }
}
#####################################################################
#####################################################################
...
#####################################################################
# # Example paramaters
# DATA1 = DATA2 = x.inb.cal[[1]][[1]][[1]]
# q = 0
# nknot = 5
# range.grid = c(0, 18)

# ... is ignored
semimetric.maximum <- function(DATA1, DATA2, q, ...)
{# Derive the data
  for (i in rep_len(1, length.out = q)) {
    DATA1 <- t(apply(DATA1, 1, diff))
    DATA2 <- t(apply(DATA2, 1, diff))
  }
  
  # Calculate the semimetric
  max_1 = apply(DATA1, 1, max)
  max_2 = apply(DATA2, 1, max)
  outer(max_1, max_2, function(x, y) {
    abs(x - y)
  })
}

# ... is ignored
semimetric.minimum <- function(DATA1, DATA2, q, ...)
{# Derive the data
  for (i in rep_len(1, length.out = q)) {
    DATA1 <- t(apply(DATA1, 1, diff))
    DATA2 <- t(apply(DATA2, 1, diff))
  ...
  # Calculate the semimetric
  min_1 = apply(DATA1, 1, min)
  min_2 = apply(DATA2, 1, min)
  outer(min_1, min_2, function(x, y) {
    abs(x - y)
  })
}
...
# helper function for semimetric.dtw
warping_path_dist <- function(x, y) {
  dist <- dtw(x, y)
  # Calculate mean absolute deviation of the indices of the warping functions
  mean(abs(dist$index1 - dist$index2))
}

# ... is ignored
semimetric.dtw <- function(DATA1, DATA2, q = 0, type = c("warped_data", "warping_path"), ...) {
  require("dtw")
  # Make sure that type is only "warped_data" or "warping_path"
  type = match.arg(type)
  ...
  subs = TRUE
  if(subs == TRUE) {
  subseq <- seq(1, ncol(DATA1), by = 5)
  subseq2 <- seq(1, ncol(DATA2), by = 5)
  }
  
  # Calculate the semimetric
  if(type == "warped_data") {
    # Distance of the warped functions
    proxy::dist(DATA1[, subseq], DATA2[, subseq2], method = "DTW")
  } else if(type == "warping_path") {
    # Distance of the warping functions
    t(apply(DATA1[, subseq], 1, function(x) {
      apply(DATA2[, subseq2], 1, warping_path_dist, x)
    }))
  }
}
# wrapper function for warping path distance
semimetric.dtw_path <- function(DATA1, DATA2, q = 0, ...) {
  semimetric.dtw(DATA1, DATA2, q = q, type = "warping_path", ...)
}
# # Examples
# semimetric.dtw(DATA1[1:3,], DATA2[1:3,])
# semimetric.dtw(DATA1[1:3,], DATA2[4:5,])
# 
# semimetric.dtw(DATA1[1:3,], DATA2[1:3,], type = "warping_path")
# semimetric.dtw(DATA1[1:3,], DATA2[4:5,], type = "warping_path")
