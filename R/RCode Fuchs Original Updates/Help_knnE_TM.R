...
Q.vec.lsei <- try(lsei(A = A, B = b, G = G, H = hh, E = NULL, F = NULL)$X)
# if (length(Q.vec.lsei) > 0) Cs <- Q.vec.lsei

################################################################################
# TM Robust (?) Version using solve.QP

# New Code including the restriction \sum Cs = 1
Cs <- vector()
...
# The two versions do almost the same thing:
all.equal(Cs, Q.vec.lsei)


# # TM: Optimierung (6) des quadratischen Optimierungsproblems
# # es funktioniert bis 100, bei 101 stoppt es
# Q.vec.lsei <- try(lsei(A = A[50:101,], B = b[50:101], G = G, H = hh, E = E, F = f)$X)
# if (length(Q.vec.lsei) > 0) Cs <- Q.vec.lsei
